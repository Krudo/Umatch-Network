...
var networkStopTick = false;		//when the alpha value drops to display the graph, do we stop the nodes from animating?
...
jQuery(document).ready(function ($) {
    ...
    setTimeout(function () {
        ...
        end of disabled by robert*/
        //added by robert instead of the ajax request to immitate the delay
        window.setTimeout(() => {
            buildName_Description_tripleObjectStore(data3); //added by robert - instead of buildNameStore+buildDescriptionStore+buildTripleStore
...
function dataAnalysis() {
    ...
    for (x in tripleObject) {	//each x here is a person
        ...
        size = descObject[x].conCounters['3'] + descObject[x].conCounters['2'] + descObject[x].conCounters['1'];
        size = importantFactor(x, size);
        //console.log(x);
        var sizeObj = {};
        sizeObj.node = x;
        sizeObj.size = size;
  ...
  .append("svg:path")
    ...
    */
	...
	.enter().insert("line", "circle.node")
	  ...
	  .style("stroke-width", function (d) { return edgeStrokeWidth(d); }) // ("stroke-width", 3)
    ...
    nodeEnter.append("circle")
		...
		.style("stroke", "white") //function (d, i) { return returnNodeColor(d); })
		.style("stroke-width", 5);//function (d) { return returnNodeStrokeWidth(d); });
    ...
    nodeEnter.append("svg:image")
		  ...
		  .attr("xlink:href", function (d) {
		      //return (descObject[d.id].pic_url)? descObject[d.id].pic_url : "menu/no_image.png"
...
function returnNodeColor(d) {
    ...
    //var important = isImportant(d.id)
    //if (important===1) return 'yellow';
    //else if (important===2) return 'red';
...
function returnNodeSize(d) {
    ...
    if (visMode == "person") {
        ...
        //return Math.round(13*d.connections^(-0.654));
        //return Math.round(d.connections);
        //console.log(largestConnection);
        //var max = Math.max(largestNodes[largestNodes.length-1].size,largestNodes[0].size);
        //console.log('cons:',d.connections,'max',largestConnection,'multiplier',(d.connections/largestConnection+0.1)*2.5,'result',15*(d.connections/largestConnection+0.3)*1.5)
        //console.log(visMode);
        if (visMode === "wave")
            return calcNodeSize(d);
        else if (visMode === "free")
            return Math.round(10 * (d.connections / largestConnection + 0.1) * 2.5);
...
function showPopup(d, cords) {
	...
	*/
    /*
	var useImage = (descObject[d.id].pic_url)? descObject[d.id].pic_url : "menu/no_image.png"
	console.log('start')
	jQuery("#popUp").append(
		...
		$("<div>")//name
			...
			.css("top","13px") 
			.css("font-size","16px")
			.css("font-family"," 'Calibri Bold', 'Calibri Regular', 'Calibri'")
			.css("font-weight","700")
		...
		$("<div>")
			...
			//.css("text-align","left")
			.attr("id","popupDesc")
			.html('<p>'+descObject[d.id].comp_name+'</p><p>'+descObject[d.id].pos+'</p>')
	).append(
		$("<div>")			 
			.css("font-size","10px")
			.css("width","138px")
...
*/
    ...
    /*
    removed by robert - no data for this feauture
...
function changeVisMode(changeTo) {
    ...
    baseNodes.forEach((node) => { node.lock = false });


    if (changeTo == "person") {
        updateHeader(usePerson);

        $('#back-to-full-map').show();
        History.pushState({ state: idLookup[usePerson] }, document.title, "?person=" + idLookup[usePerson]);
        ...
        setTimeout(function () {
            ...
            $('#content1').removeClass('fullwidth').delay(10).queue(function (next) {
                $(this).addClass('fullwidth');
                next();
            });
            return false;
        ...
        setTimeout(function () {
            ...
            $('#content2').removeClass('fullwidth').delay(10).queue(function (next) {
                $(this).addClass('fullwidth');
                next();
            });
            return false;
        ...
        setTimeout(function () {
            ...
            $('#content3').removeClass('fullwidth').delay(10).queue(function (next) {
                $(this).addClass('fullwidth');
                next();
            });
            return false;
        ...
        setTimeout(function () {
            ...
            $('#content4').removeClass('fullwidth').delay(10).queue(function (next) {
                $(this).addClass('fullwidth');
                next();
            });
            return false;
...
function buildDynamicList() {
    ...
    for (x in listNodes) {
        ...
        var descText = getDescText(listNodes[x]);
		...
		(
			$("<div>")
				...
				(
					$("<img>")
						.attr("src", function () {
						    return (descObject[listNodes[x].id].pic_url) ? descObject[listNodes[x].id].pic_url : ''
						...
						.css("visibility", function () {
						    return (descObject[listNodes[x].id].pic_url) ? 'visible' : 'hidden'
						    /*if (fileNames.indexOf(id_img+'.png')!=-1){
								return "visible"	
							}else{
								return "hidden"								
...
function edgeColor(d) {
    ...
    if (typeof d.connections == 'undefined') {
        return getConnectionColor(d.source, d.target);
        d = d.source;
    }
...
function linkStrength(d) {
    ...
    if (visMode == "person") {
        //console.log('getConnectionLevel/10',getConnectionLevel(d.source.id,d.target.id)/10,'getLinkStrength',getLinkStrength(d.source.id,d.target.id));
        //		console.log(getLinkStrength11(d.source,d.target));
        //	return getConnectionLevel(d.source.id,d.target.id)/10;		
        return getLinkStrength(d.source, d.target);
    }
...
function windowResize() {
    ...
    visHeight = $(window).height() - 126;
    console.log('vis widthXheight:', visWidth, visHeight)
